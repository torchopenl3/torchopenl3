...
class CustomSpectrogram(nn.Module):
  ...
  def __init__(self, type, n_fft, n_hop, asr):
    ...
    self.stft = nnAudio.Spectrogram.STFT(
        n_fft=n_fft, win_length=None, freq_bins=None, hop_length=n_hop, window='hann',
        freq_scale='no', center=False, pad_mode=None, iSTFT=False,
        fmin=0, fmax=asr//2, sr=asr, trainable=False,
        output_format="Magnitude", verbose=False
    )
    if self.type == "mel128":
      self.mel_basis = librosa.filters.mel(sr=asr, n_fft=n_fft, n_mels=128, fmin=0, fmax=asr//2, htk=True, norm=1)
      self.mel_basis = T(self.mel_basis, dtype = torch.float32)
    elif self.type == "mel256":
      self.mel_basis = librosa.filters.mel(sr=asr, n_fft=n_fft, n_mels=256, fmin=0, fmax=asr//2, htk=True, norm=1)
      self.mel_basis = T(self.mel_basis, dtype = torch.float32)
  ...
  def amplitude_to_decibel(self, x, amin=1e-10, dynamic_range=80.0):
      ...
      log_spec = T(10.0, dtype = torch.float32) * torch.log(torch.maximum(x, T(amin))) / torch.log(T(10.0, dtype = torch.float32))
      if x.ndim > 1:
